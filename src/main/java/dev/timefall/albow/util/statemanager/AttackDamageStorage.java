package dev.timefall.albow.util.statemanager;
...
public class AttackDamageStorage {
    ...
    public static void storeDamage(LivingEntity player, float storedDamage) {
        UUID uuid = player.getUuid();
        LAST_STORED_AMOUNT.put(player.getUuid(), storedDamage);
    ...
    public static void checkAndClearExpired(LivingEntity player) {
        ...
        if (LAST_ATTACK_TIME.containsKey(uuid)) {
            ...
            if (currentTime - LAST_ATTACK_TIME.get(uuid) > CleanlinessHelper.albow$calculateTicksFromSeconds(3)) {
